...
import { aiCaptionSchema, formatZodError } from '@/lib/validation/schemas';
import { generateAiContent, buildCaptionPrompt } from '@/lib/ai';
...
export async function POST(req: NextRequest) {
  ...
  const rate = await checkRateLimit('ai-tools', auth.user.id, { points: 60, durationSeconds: 3600 });
  ...
  try {
    const result = await generateAiContent({ prompt: buildCaptionPrompt(parsed.data), maxTokens: 300 });
