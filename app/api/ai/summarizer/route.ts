...
import { aiNewsSummarySchema, formatZodError } from '@/lib/validation/schemas';
import { generateAiContent, buildNewsSummaryPrompt } from '@/lib/ai';
...
export async function POST(req: NextRequest) {
  ...
  const parsed = aiNewsSummarySchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) return jsonError('Validation failed.', 422, formatZodError(parsed.error));

  try {
    const result = await generateAiContent({ prompt: buildNewsSummaryPrompt(parsed.data) });
    return jsonOk({ ...result, requiresApproval: true, sourceUrl: parsed.data.sourceUrl, sourceName: parsed.data.sourceTitle });
  } catch (err) {
