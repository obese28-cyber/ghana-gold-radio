...
import { aiTop10CommentarySchema, formatZodError } from '@/lib/validation/schemas';
import { generateAiContent, buildTop10CommentaryPrompt } from '@/lib/ai';
...
export async function POST(req: NextRequest) {
  ...
  const parsed = aiTop10CommentarySchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) return jsonError('Validation failed.', 422, formatZodError(parsed.error));

  try {
    const result = await generateAiContent({ prompt: buildTop10CommentaryPrompt(parsed.data) });
