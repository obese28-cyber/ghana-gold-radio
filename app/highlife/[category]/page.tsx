...
async function getCategoryAndPosts(slug: string) {
  const category = await findCategoryBySlugAndType(slug, 'highlife');
...
export async function generateMetadata({ params }: { params: { category: string } }) {
  ...
  return buildMetadata({ title: category.name, description: category.description ?? undefined, path: `/highlife/${params.category}` });
}

export default async function HighlifeCategoryPage({ params }: { params: { category: string } }) {
