...
async function getCategoryAndPosts(slug: string) {
  const category = await findCategoryBySlugAndType(slug, 'diaspora_update');
...
export async function generateMetadata({ params }: { params: { category: string } }) {
  ...
  return buildMetadata({ title: category.name, description: category.description ?? undefined, path: `/diaspora/${params.category}` });
}

export default async function DiasporaCategoryPage({ params }: { params: { category: string } }) {
