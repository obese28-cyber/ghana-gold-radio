...
async function getCategoryAndPosts(slug: string) {
  const category = await findCategoryBySlugAndType(slug, 'afrobeats_hiplife');
...
export async function generateMetadata({ params }: { params: { category: string } }) {
  ...
  return buildMetadata({ title: category.name, description: category.description ?? undefined, path: `/afrobeats/${params.category}` });
}

export default async function AfrobeatsCategoryPage({ params }: { params: { category: string } }) {
